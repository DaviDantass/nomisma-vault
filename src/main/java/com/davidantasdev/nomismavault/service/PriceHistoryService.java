...
public class PriceHistoryService {
  ...
  /** Retorna snapshots observados pelo sistema, não uma série histórica oficial de mercado. */
  ...
  public List<PriceHistoryResponse> findByAssetIdAndDateRange(
      Long assetId, LocalDate startDate, LocalDate endDate) {
    if (!assetRepository.existsById(assetId)) {
      throw new ResourceNotFoundException("Ativo não encontrado");
    }
    return priceHistoryMapper.toResponseList(
        priceHistoryRepository.findByAssetIdAndDateBetweenOrderByDateAsc(
  ...
  public PriceHistoryResponse findLatestByAssetId(Long assetId) {
    ...
    PriceHistory priceHistory =
        priceHistoryRepository
            .findTopByAssetIdOrderByDateDesc(assetId)
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
                        "Nenhum histórico de preço encontrado para este ativo"));
    return priceHistoryMapper.toResponse(priceHistory);
  }

  @Transactional
  /** Registra no máximo um snapshot por ativo e data. */
  ...
  public PriceHistoryResponse update(Long id, PriceHistoryRequest request) {
    ...
    priceHistoryRepository
        .findByAssetIdAndDate(request.assetId(), request.date())
        .ifPresent(
            existing -> {
              if (!existing.getId().equals(id)) {
                throw new BusinessException(
                    "Já existe um registro de preço para este ativo nesta data");
              }
            });
