...
public class PortfolioService {
  ...
  public Page<PortfolioResponse> findAllByUser(Long userId, Pageable pageable) {
    ...
    return portfolioRepository.findAllByUser(user, pageable).map(portfolioMapper::toResponse);
  }
  ...
  public PortfolioSummaryResponse getSummary(Long userId, Long portfolioId) {
    ...
    Portfolio portfolio =
        portfolioRepository
            .findByIdAndUser(portfolioId, user)
            .orElseThrow(() -> new ResourceNotFoundException("Portfolio not found for this user"));
  ...
  public PortfolioResponse updatePortfolio(
      Long userId, Long portfolioId, PortfolioRequest request) {
    User user =
        userRepository
            .findById(userId)
            .orElseThrow(() -> new ResourceNotFoundException("Usuário não encontrado"));
