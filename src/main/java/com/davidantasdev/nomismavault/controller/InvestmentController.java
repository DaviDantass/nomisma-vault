...
public class InvestmentController {
  ...
  @GetMapping
  public ResponseEntity<PageResponse<InvestmentResponse>> findAllInvestmentsByPortfolio(
      @PathVariable Long portfolioId, Pageable pageable) {
    return ResponseEntity.ok(
        PageResponse.from(investmentService.findAllByPortfolio(portfolioId, pageable)));
  }

  @GetMapping("/{investmentId}/pnl")
  public ResponseEntity<InvestmentWithPnLResponse> getInvestmentPnL(
      @PathVariable Long portfolioId, @PathVariable Long investmentId) {
    return ResponseEntity.ok(investmentService.getInvestmentWithPnL(portfolioId, investmentId));
  }

  @GetMapping("/{investmentId}")
  public ResponseEntity<InvestmentResponse> findInvestmentById(
      @PathVariable Long portfolioId, @PathVariable Long investmentId) {
    return ResponseEntity.ok(investmentService.findById(portfolioId, investmentId));
  }
}
